...
import weld_stitch.weld._teardrop as td
...
def make_distance_image(case="30",W=100.0,plane='x-y',with_contours=True,fixed_coordinate_value=0):
    ...
    for p in range(num_points):
        x=xyz[p,0]
        y=xyz[p,1]
        # Teardrop is symmetric about 'y-axis'; use absolute value of 'x'
        u,distance=td._get_teardrop2d_cpp(fabs(x),y,capsule);
    ...
    if with_contours is True:
        ...
        CS=ax.contour(d.reshape(dim_2d[1],dim_2d[0]),levels,colors='k',origin='lower',linewidths=2,aspect='equal',extent=extent_2d)
    # turn axes 'on' or 'off'
...
def get_unit_pool(case="20",W=1.0):
    ...
    # Pool position
    yp=td._get_teardrop2d_position(capsule)
    s="Pool position yp={0:6.4f}"
    print(s.format(yp))
    return capsule,td
...
def plot_curve(case="20",pool_width=100.0):
    ...
    # SYMMETRY: Plot curve for x<0
    ax.plot(-c[0,:],c[1,:],color='b',marker='None',linewidth=t)
    ax.axis('equal')
    fig.subplots_adjust(left=0.16,bottom=.14,top=0.92)
    fig.show()
...
def plot_cpp_functional(x,y,umin,umax,case="20"):
    ...
    # Nice example:  plot_cpp_functional(.5,0,.1,.4,case="20")
    a=umin
    b=umax
    num_points_curve=100
