   ...
   class TeardropCurve {
      ...
      public:
         ...
         void compute_curve(double u, double *curve) const {
            ...
            const double ax=P[ai][0];
            const double cx=P[ci][0];
            const double dx=P[di][0];

            const double by=P[bi][1];
            const double cy=P[ci][1];
            const double dy=P[di][1];

            // X-component of curve
            c[0]=ax*um1_4+4*ax*um1_3u+6*cx*um1_2u2+4*dx*um1u3+dx*u4;

            // Y-component of curve
            c[1]=4*by*um1_3u+6*cy*um1_2u2+4*dy*um1u3;
         }
         ...
         void compute_dcurve(double u, double *dcurve) const {
            ...
            dc[0]=12*(cx-ax)*um1_2u+12*(dx-cx)*um1u2;

            // Y-component of derivative of the curve
            dc[1]=4*by*um1_3+12*(cy-by)*um1_2u+12*(dy-cy)*um1u2-4*dy*u3;
         }
         ...
         void compute_ddcurve(double u, double *ddcurve) const {
            ...
            ddc[0]=12*(cx-ax)*um1_2+(24*ax-48*cx+24*dx)*um1u+12*(cx-dx)*u2;

            // Y-component of 2nd derivative of the curve
            ddc[1]=(12*cy-24*by)*um1_2+(24*by-48*cy+24*dy)*um1u+(12*cy-24*dy)*u2;
         }
         ...
         double get_pool_length() const {
            double xmin,xmax;
            // Length of pool is controlled by x-component 
            //   of first and last control point
            xmin=P[0][0];
            xmax=P[4][0];
            double pool_length=xmax-xmin;
            return pool_length;
         }
         ...
         double get_pool_width() const {
            ...
            double ymax=std::fabs(c[1]);
            double ymin=-ymax;
            double pool_width=ymax-ymin;
            return pool_width;
         }

         double get_pool_position() const {
            // Pool position is defined as average along x-axis
            double xmin,xmax;
            xmin=P[0][0];
            xmax=P[4][0];
            double pool_position=0.5*(xmax+xmin);
         ...
         TeardropCurve(const vector< vector<double> >& control_points, double pool_width, bool normalize_pool_position=false) :
            P(control_points) {
            {
               // Normalize set of control points; set 'pool width' ;
               ...
               if(normalize_pool_position) {
                  // Positions pool at x=0.0
                  double dx=0.0;
                  double xp=get_pool_position();
                  dx-=xp;
                  move_curve(dx);
               }
            }
      ...
      private:
         ...
         void move_curve(double dx) {
            // This function CHANGES 'this' curve.
            // Moves curve in 'x-direction' by 'dx'.
            // NOTE that 'delta' control points do not require an update
            //    since a move has no effect on them.
            const int N=4;
            for(int i=0;i<=N;i++){
               vector<double> pi=P[i];
               // Only move 'x-coordinates'
               pi[0]+=dx;
               P[i]=pi;
            }
         }
      ...
      public:
         ...
         double predict_cpp_parametric_coordinate(double x, double y, double du) const {
            ...
            if (x>xm){
               ...
               if(y<tolerance) return umax;
            }
            else {
               umin=0;
               umax=um;
               // If point is on weld axis
               if(y<tolerance) return umin;
            }
         ...
         std::tuple<double,double> closest_point(double x, double y) const {
            ...
            //std::cout << "PREDICTOR u0 = " << std::scientific << std::setw(15) << u0 << std::endl;
            ...
            double dr[]={x-rho[0],y-rho[1]};
            {
               ...
               // Note that curve is moving in a clockwise fashion.
               // rho_1 cross k produces a vector that points towards interior.
               double n[]={rho_1[1],-rho_1[0]};
               double dot=(n[0]*dr[0]+n[1]*dr[1]);
               // If above dot product is positive then (x,y) defines
   ...
   class Teardrop : public TeardropCurve, public PoolShape {
      ...
      public:
         ...
         virtual double distance(const double *XYZ) const {
            ...
            // Model uses symmetry about x-z plane;
            // Model assumes and requires y-coordinates are positive.
            // Because of symmetry, distance(y)=distance(-y)
            double xyz[]={(*XYZ), std::fabs(*(XYZ+1)), *(XYZ+2)};
      ...
      private:
         void compute_closest_point(const double *xyz, const double *u0, double *U) const {
             ...
             *
             * U: 2 component vector solution: parametric coordinates of closest point on surface.
             *    
             * Outputs
             * -------
